...
Input.fromBufferReader = function (br, params) {
  ...
  if (params && params['hex']) {
    ...
    var sigString = inputList[params['index']].script_sig.slice(4, (parseInt(inputList[params['index']].script_sig.slice(2, 4), 16)) * 2 + 2)
    var signatures = []
    var utxo = params['utxos'].find(function(element) {
      return element['txid'] === input.prevTxId.toString('hex') && input.outputIndex === element['vout'];
    });
    if (utxo['public_key']) {
